...
public class RedAudienceSideTrajectories implements AudienceSideTrajectories {
    ...
    int propPosOrdinal;
    private final Pose2d startPose = new Pose2d(-35, -63, Math.toRadians(-90));

    public RedAudienceSideTrajectories(SampleMecanumDrive drive, PropPos propPos) {
        this.drive = drive;
        fillVariables();
    ...
    private void fillVariables() {
        goToPixelStack = new TrajectorySequence[]{
                ...
                drive.trajectorySequenceBuilder(startPose)
                        .setReversed(true)
                        .lineToLinearHeading(new Pose2d(-43, -21, Math.toRadians(0)))
                        .setReversed(false)
                        .splineToLinearHeading(new Pose2d(-56.5, -25, Math.toRadians(-170)), Math.toRadians(180),
                                SampleMecanumDrive.getVelocityConstraint(AutonomousConstants.maxStackHitSpeed, DriveConstants.MAX_ANG_VEL, DriveConstants.TRACK_WIDTH),
                                SampleMecanumDrive.getAccelerationConstraint(DriveConstants.MAX_ACCEL)
                        )
                        .lineToSplineHeading(new Pose2d(-55.5, -15,Math.toRadians(180)))
                        .build(),

                drive.trajectorySequenceBuilder(startPose)
                        .lineToLinearHeading(new Pose2d(-40, -36, Math.toRadians(0))) // needed to not hit the truss
                        .lineTo(new Vector2d(-35, -34))
                        .splineToLinearHeading(new Pose2d(-56, -24, Math.toRadians(-170)), Math.toRadians(0),
                                SampleMecanumDrive.getVelocityConstraint(AutonomousConstants.maxStackHitSpeed, DriveConstants.MAX_ANG_VEL, DriveConstants.TRACK_WIDTH),
                                SampleMecanumDrive.getAccelerationConstraint(DriveConstants.MAX_ACCEL)
                        )
        ...
        goBackstage = new TrajectorySequence[]{
                ...
                drive.trajectorySequenceBuilder(goToPixelStack[1].end())
                        ...
                        .splineTo(new Vector2d(43, -27), Math.toRadians(0))
                        .splineTo(new Vector2d(51, -27), Math.toRadians(0),
                                SampleMecanumDrive.getVelocityConstraint(AutonomousConstants.maxPixelPlacementSpeed, AutonomousConstants.maxAngularPixelPlacementSpeed, DriveConstants.TRACK_WIDTH),
                                SampleMecanumDrive.getAccelerationConstraint(DriveConstants.MAX_ACCEL)
                        )
                        .build(),
        ...
        goToYellowPixelPlacement = new TrajectorySequence[]{
                ...
                drive.trajectorySequenceBuilder(goBackstage[1].end())
                        .lineToConstantHeading(new Vector2d(51.5, -35)) //-37,5
                        .waitSeconds(0.5)
                        .build(),

                drive.trajectorySequenceBuilder(goBackstage[2].end())
                        .lineToConstantHeading(new Vector2d(52, -43.5))
                        .waitSeconds(0.3)
                        .build()
        };
